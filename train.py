...
from dataloaderT import Seq2DataLoader
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.distributed:
        ...
        for key,item in model.items():
            model[key] = nn.SyncBatchNorm.convert_sync_batchnorm(model[key])
            model[key] = model[key].cuda(args.gpu)
            model[key] = torch.nn.parallel.DistributedDataParallel(model[key], device_ids=[args.gpu], output_device=args.gpu,
...
def train(model, args, epochs=10, experiment_name="DeepLab", lr=0.0001, root=".", device=None,
    ...
    if args.same_lr:
        ...
        print("Using diff LR")
        m = model['UnetAdaptiveBins'].module if args.multigpu else model
        params = [{"params": m.get_1x_lr_params(), "lr": lr / 10},
                  {"params": m.get_10x_lr_params(), "lr": lr}]
        for key,item in model.items():
            if key != 'UnetAdaptiveBins':
                params += list(model[key].parameters())  # 需要测试
...
def predict_poses(args,input,feature):
    output = {}
    # pose_feats = {f_i: inputs["color_aug", f_i, 0] for f_i in self.opt.frame_ids}
    pose_feats = {f_i:input["color",f_i] for f_i in args.frame_ids}
