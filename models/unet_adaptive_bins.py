...
class UnetAdaptiveBins(nn.Module):
    ...
    def forward(self, x, **kwargs):
        ...
        out = self.conv_out(range_attention_maps) # N通道
    ...
    @classmethod # python的classmethod叫做python的类方法，是只需要在普通方法上加上@classmethod这样一个装饰器就可以
    # cls所代表的东西代表的其实是一个类本身，它可以用来调用类自己的属性和类自己的方法。
    def build(cls, n_bins, **kwargs):
        basemodel_name = 'tf_efficientnet_b5_ap'

        print('Loading base model ()...'.format(basemodel_name), end='') #print默认是打印一行，结尾加换行。end=' '意思是末尾不换行，加空格
        basemodel = torch.hub.load('rwightman/gen-efficientnet-pytorch', basemodel_name, pretrained=True)
        print('Done.')

        # Remove last layer
        # 删除的方法是 将最后的两层变为Identity，是一种将输入直接输出的层，网络整体层数不变
...
if __name__ == '__main__':
    models = UnetAdaptiveBins.build(100)
